...
uv pip install --upgrade --default-index "${PIP_INDEX_URL}" pip pkginfo || \
  uv pip install --upgrade --default-index https://pypi.org/simple pip pkginfo
...
uv pip install --no-binary :all: --default-index "${PIP_INDEX_URL}" psutil || \
  uv pip install --no-binary :all: --default-index https://pypi.org/simple psutil
uv pip install --upgrade --default-index "${PIP_INDEX_URL}" \
  "setuptools<=81" \
  packaging \
  Cython \
  wheel \
  uv \
  nvidia-ml-py || \
  uv pip install --upgrade --default-index https://pypi.org/simple \
    "setuptools<=81" \
    packaging \
    Cython \
    wheel \
...
# Try with the specified PIP_INDEX_URL first, fallback to default PyPI if it fails
uv pip install --upgrade --default-index "${PIP_INDEX_URL}" twine || \
  uv pip install --upgrade --default-index https://pypi.org/simple twine
